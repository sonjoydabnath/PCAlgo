...
/**
*  Pre-Generating all Divisors Upto MAX in O(nlogn)
*  for 1e6 RT: 0.27sec and MAX multiply by 15 is OK
*  for 137 RT: 3.50sec and MAX multiply by 17 is OK
**/
namespace Divs {

const int MAX = 1000000;
int D[MAX * 15], NextDivInx[MAX * 15];
int FirstDivInx[MAX + 7];
vector<int>  Divisors;
void PreProc() {
    int Index = 0;
    for( int i = 1; i <= MAX; i++ ) {
        for( int j = i; j <= MAX; j += i ) {
            D[++Index] = i;
            NextDivInx[Index] = FirstDivInx[j];
            FirstDivInx[j] = Index;
        }
    }
}
vector<int> getDivs( int N ) {
    Divisors.clear();
    for( int i = FirstDivInx[N]; i != 0; i = NextDivInx[i] )
...
//    WRITE("out.txt");
    ...
    Divs::PreProc();
    vi Divisors = Divs::getDivs( Divs::MAX );
    debug( Divisors );

    rep1( i, 100 ) {
        Divisors = Divs::getDivs( i );
        debug( Divisors );
    }
