...
void update_tree( int node, int a, int b, int i, int j, i64 value ) {
    ...
    tree[node] = ( tree[node * 2] + tree[node * 2 + 1] ); // summing up
}

/* Query tree to get sum of element value within range [i, j]*/

i64 query_tree( int node, int a, int b, int i, int j ) {
    ...
    if( a >= i && b <= j ) // Current segment is totally within range [i, j]
        return tree[node];
    i64 q1 = query_tree( node * 2, a, ( a + b ) / 2, i, j ); // Query left child
    i64 q2 = query_tree( 1 + node * 2, 1 + ( a + b ) / 2, b, i, j ); // Query right child
    i64 res = ( q1 + q2 ); // Return final result
    return res;
}
