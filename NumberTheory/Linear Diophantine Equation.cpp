...
pair< i64, pll> ext_gcd( i64 a, i64 b ) { if( b == 0 ) return make_pair( a, make_pair( 1LL, 0LL ) ); pair<i64, pll> t = ext_gcd( b, a % b ); return make_pair( t.first, make_pair( t.second.second, t.second.first - a / b * t.second.second ) ); }
i64 ModInvNonPrimeM( i64 N, i64 M ) { pair< i64, pll > r = ext_gcd( N, M ); r.second.first %= M; if( r.second.first < 0 ) r.second.first += M; return r.second.first; }
...
pair< i64, pll> LinearDiophantine( i64 A, i64 B, i64 C ) {
    pair< i64, pll > r = ext_gcd( A, B );
    if( C % r.first ) return mp( 0LL, mp( 0LL, 0LL ) );
    r.second.first *= C / r.first;
    r.second.second *= C / r.first;
    return r;
}
