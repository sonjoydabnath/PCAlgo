...
/**Problem: https://codechef.com/JAN16/CYCLRACE
*  Convex Hull Trick Solution
*  Implemented by SONJOY(sonjbond)
*  Reference: http://wcipeg.com/wiki/Convex_hull_trick
**/

struct CYC {
    i64 currPosition, currSpeed, currTime;
    CYC( i64 cp = 0, i64 cs = 0, i64 ct = 0 ) {
        currPosition = cp;
        currSpeed = cs;
        currTime = ct;
    }
...
#define C second //constant c
#define F first
#define S second
//where a line is denoted by y= mx + c equation
vector< Line > UpHull;  // to keep track of Upper Hull
int LastBestInx;

bool isBad(  pair<i64, i64> L1, pair<i64, i64> L2, pair<i64, i64> L3 ) {
    //L2 to be removed if condition is met
    /*
    Now we have to calculate intersect point of (L1, L2) and (L1, L3)
    Actually we need only X-coordinate of these intersection points.
    Lets say X1 = IntersectionPointX(L1, L2)
    Lets say X2 = IntersectionPointX(L1, L3)
    if(X1 is in right side of X2 that means X1 > X2) then we must remove L2 line because it's No More a perfect line for our solution, it's actually out of Hull's, it's below to Hull's any lower sides,

    Now, how to calculate X1 and X2?? (!!!) :O
    Oh! that's that not much hard, as we can see every line is actually Y = MX + C.
    Say, L1 is Y = M1*X + C1, L2 is Y = M2*X + C2, we have to find soln(X,Y) these two equations where Lines will cross each other
    as Lines will cross we can write M1*X + C1 = M2*X + C2
    Or, X = (C2 - C1)/(M1 - M2)
...
void addLine( pair<i64, i64> currLine ) {
    int sz = UpHull.size();
    while( sz > 1 && isBad( UpHull[sz - 2], UpHull[sz - 1], currLine ) ) {
        UpHull.pop_back();
        sz--;
    }
    UpHull.pb( currLine );
...
//    WRITE("out.txt");
    ...
    rep0( j, q ) {
        ...
        if( QueryType == 1 ) {
            read( CycId );
            read( NewSpeed );
            Cyclists[CycId].currPosition += Time * ( Cyclists[CycId].currSpeed - NewSpeed );
            Cyclists[CycId].currSpeed = NewSpeed;
            Cyclists[CycId].currTime = Time;
            //Convert Cyclist into Lines( Y = MX + C ) where
            //C = Position or cutting point of Y-axis, Slope M = NewSpeed
            Line tmp = Line( NewSpeed, Cyclists[CycId].currPosition );
            addLine( tmp ); // putting this line into graph and making a Upper Hull then at any time we calculate Max Position using those lines from Upper Hull, putting X = Time, MaxPosition = M * Time + C
        }
        else if( QueryType == 2 ) {
            cout << query( Time ) << "\n";
        }
