...
bool compareX( const PT &p1, const PT &p2 ) {
    if( p1.x != p2.x )
        return p1.x < p2.x;
    return p1.y < p2.y;
}
...
double ClosestPairOfPoint( vector<PT> Points ) {
    sort( all( Points ), compareX );
    set<PT, compareY> Active;
    int tail = 0;
    double h = DINF;
    for( int i = 0; i < Points.size(); i++ ) {
        while( Points[i].x - Points[tail].x > h ) {
            Active.erase( Points[tail] );
            tail++;
        }
        _itr( Active ) le, ri;
        le = lower_bound( all( Active ), PT( -DINF, Points[i].y - h ), compareY() );
        ri = upper_bound( all( Active ), PT( +DINF, Points[i].y + h ), compareY() );
        for( ; le != ri; le++ ) {
            h = min( h, dist( *le, Points[i] ) );
        }
...
//    WRITE("out.txt");
    ...
    cout<<fixed<<setprecision(6)<<ClosestPairOfPoint(Points)<<"\n";
