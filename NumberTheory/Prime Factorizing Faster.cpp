...
void preProcess() {
    ...
    for( int j = 0; j <= MAX; j++ )if( status[j] == 0 )status[j] = j;
}

//void factorizeArray( int sz, int x[], int xf[] ) {
//    for( int i = 0; i < sz; i++ ) {
//        for( int j = x[i]; j > 1; j = j / status[j] ) {
//            xf[status[j]]++;
//        }
//    }
//}
...
//    WRITE("out.txt");
    ...
    preProcess();
    for( i = 1; i <= MAX; i++ ) {
        NumberOfPrimeFactor[i] = FactorCount( i );
        CummiSum[i] = CummiSum[i - 1] + NumberOfPrimeFactor[i];
        //debug( i, NumberOfPrimeFactor[i], CummiSum[i] );
    }

    cin >> tc;
    while( tc-- ) {
        CI( a, b );
        cout << CummiSum[a] - CummiSum[b] << "\n";
    }
